...
\verb|-f| is to read them when the container is printing:
\begin{codetemplate}{}
\begin{verbatim}
...
To retrieve logs from all containers:
\begin{codetemplate}{}
\begin{verbatim}
...
To retrieve logs from previous pods from a deployment:
\begin{codetemplate}{}
\begin{verbatim}
...
Now we should be able to connect to out cluster and see our node!
\begin{codetemplate}{}
\begin{verbatim}
$ kubectl get nodes
\end{verbatim}
...
If you need to recreate a token:
\begin{codetemplate}{}
\begin{verbatim}
...
Check on the main control-plane node:
\begin{codetemplate}{}
\begin{verbatim}
$ kubectl get nodes
\end{verbatim}
...
\subsection{Nodes}
To check the status of the nodes, you will need to find on \textbf{conditions}:
\begin{codetemplate}
\begin{verbatim}
...
When \verb|kubelet| is not ready you will see something like:

\begin{figure}[H]
    \includegraphics[width=\textwidth]{pictures/kubelet1.png}
\end{figure}

When the \verb|kubelet| is running, the different flags mean:
\begin{itemize}
    \item \verb|OutOfDisk:| indicates whether the node is out of disk space required to add new pods. Kubernetes monitors the disk space on each node, and if the disk is almost full, the OutOfDisk status will be set to \verb|True|, preventing additional pods from being scheduled to avoid disk exhaustion.
    \item \verb|MemoryPressure:| shows whether the node is under memory pressure, meaning that available memory is running low. Kubernetes tracks this to prevent running workloads from crashing due to memory shortage.
    \item \verb|DiskPressure:| indicates that a node is running low on disk space, particularly the storage used by ephemeral data. Ephemeral storage refers to temporary storage associated with the lifecycle of pods and is commonly used for:
    \begin{itemize}
        \item Temporary files generated by applications.
        \item Logs written by applications and Kubernetes components.
        \item Container images and any local cache used by the running applications.
    \end{itemize}
    \item \verb|PIDPressure:| reflects whether the node is under process (PID) pressure, meaning it has limited capacity to create new processes. PIDPressure is a condition Kubernetes uses to detect when a node is approaching its limit on the number of processes (PIDs) it can create. Every operating system has a maximum number of process IDs it can handle, set by the \verb|kernel.pid_max| parameter
    \item \verb|Ready:| it only will be true if kubelet is properly running on the node the other specs are True
\end{itemize}

\begin{blocktemplateI}{Note}
The kubelet, which is the agent running on each Kubernetes node, continuously monitors these specs. When it detects that disk/memory/PID are above a certain threshold for ephemeral storage, it sets the conditions to True. These thresholds are configurable, and administrators can define limits based on their workload requirements.
\end{blocktemplateI}

\begin{blocktemplateII}{Note}
It can be useful to check \textbf{LastHeartbeatTime}, it will indicate the last time the node was responding.
\end{blocktemplateII}

\subsubsection{kubelet}
Remember that \textbf{kubelet} is always running like a \textbf{service} on the nodes, not like K8s resources, so to monitor its state:
\begin{codetemplate}
\begin{verbatim}
$ systemctl status kubelet
\end{verbatim}
...
The configuration file for \textbf{kubelet} is under:
\begin{codetemplate}
\begin{verbatim}
...
The certificates for \textbf{kubelet} are under:
\begin{codetemplate}
\begin{verbatim}
/var/lib/kubelet/
\end{verbatim}
...
The \textbf{kubeconfig} file used by \textbf{kubelet} is under:
\begin{codetemplate}
\begin{verbatim}
...
\begin{blocktemplate}{Note}
This can be checked watching logs of \verb|kubelet.service|
\begin{codetemplate}
\begin{verbatim}
...
\begin{codetemplate}
\begin{verbatim}
$ systemctl status kubelet | grep -i kubeconfig
\end{verbatim}
\end{codetemplate}
\begin{figure}[H]
    \includegraphics[width=\textwidth]{pictures/kubelet2.png}
\end{figure}

If the \verb|kubelet.service| is not working:
\begin{codetemplate}
\begin{verbatim}
$ journalctl -xeu kubelet | grep -i config file
\end{verbatim}
\end{codetemplate}
\end{blocktemplate}

\begin{blocktemplateIII}{Warning}
If the \verb|kubelet.service| is up and running but you see the log:
\begin{codetemplate}
\begin{verbatim}
dial tcp 192.9.193.2:6553: connect: connection refused" logger="UnhandledError"
\end{verbatim}
\end{codetemplate}
Check:
\begin{itemize}
    \item \textbf{kubelet.service kubeconfig:} \verb|/etc/kubernetes/kubelet.conf|
    \item Network connectivity 
\end{itemize}
\end{blocktemplateIII}

\subsubsection{ControlPlane Components running as Pods}
As the components are running as Pods, to check problems related to this components you should always execute:
...
\begin{codetemplate}
\begin{verbatim}
$ kubectl describe <failing_pod> -n kube-system
\end{verbatim}
\end{codetemplate}

\begin{codetemplate}
\begin{verbatim}
$ kubectl logs <failing_pod> -n kube-system
\end{verbatim}
\end{codetemplate}

\paragraph{Resources Running}
\begin{itemize}
    \item \verb|coredns-xxxxx-yyyy:| deployment
    \item \verb|etcd-controlplane:| static pod, deployed by default by kubeadm.
    \item \verb|kube-apiserver-controlplane:| static pod, deployed by default by kubeadm.
    \item \verb|kube-controller-manager-controlplane:| static pod, deployed by default by kubeadm.
    \item \verb|kube-proxy-lglqd:| daemonset, deployed by default by kubeadm.
    \item \verb|weave-net-b7vhw :| daemonset, you need to deploy it as network solution.
\end{itemize}

The static pods use to be configured in (but it is configurable in kubelet \verb|/var/lib/kubelet/config.yaml|):
\begin{codetemplate}
\begin{verbatim}
...
For deploying network solution:
\begin{codetemplate}
\begin{verbatim}
$ kubectl apply -f https://reweave.azurewebsites.net/k8s/v1.29/net.yaml
\end{verbatim}
...
\paragraph{Resources Configuration}
As they are running as Deployment, DaemonSets or Static Pods, the configuration is not on the node, it is inside configmaps. So to check / modify any configuration file you should check inside the associated \verb|ConfigMaps:|
...
\begin{figure}[H]
    \includegraphics[width=\textwidth]{pictures/getcm.png}
\end{figure}
